...
-(void)addImages{
    ...
    for (int i = 0; i < self.dataImages.count; i ++) {
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(i * MSScreenW, 0, MSScreenW, MSScreenH * 0.7)];
...
{
    // 第一页的Label1
    UILabel *firstL1 = [[UILabel alloc] init];
    firstL1.frame = CGRectMake(0, 0, 150, 50);
    firstL1.centerX = self.scrollView.contentSize.width / 3 * 0.5;
    firstL1.centerY = KSCREEN_HEIGHT * 0.75;
    firstL1.font = [UIFont systemFontOfSize:22];
    firstL1.textAlignment = NSTextAlignmentCenter;
    firstL1.textColor = KSRGBA(58, 209, 181, 1);
    firstL1.text = @"全新升级";
    [self.scrollView addSubview:firstL1];
    // 第一页的第二个Label
    UILabel *secL = [[UILabel alloc] init];
    secL.frame = CGRectMake(0, 0, 200, 30);
    secL.centerX = self.scrollView.contentSize.width / 3 * 0.5;
    secL.centerY = KSCREEN_HEIGHT * 0.8;
    secL.text = @"全新视觉体验重新上阵";
    secL.textColor = KSRGBA(86, 86, 86, 1);
    secL.textAlignment = NSTextAlignmentCenter;
    secL.font = [UIFont systemFontOfSize:14];
    [self.scrollView addSubview:secL];
    // 第二页的第一个Label
    UILabel *secPL1 = [[UILabel alloc] init];
    secPL1.frame = CGRectMake(0, 0, 150, 50);
    secPL1.centerX = self.scrollView.contentSize.width * 0.5;
    secPL1.centerY = KSCREEN_HEIGHT * 0.75;
    secPL1.font = [UIFont systemFontOfSize:22];
    secPL1.textAlignment = NSTextAlignmentCenter;
    secPL1.textColor = KSRGBA(34, 167, 232, 1);
    secPL1.text = @"购物专享";
    [self.scrollView addSubview:secPL1];
    // 第二页的第二个Label
    UILabel *secPL2 = [[UILabel alloc] init];
    secPL2.frame = CGRectMake(0, 0, 200, 50);
    secPL2.centerX = self.scrollView.contentSize.width * 0.5;
    secPL2.centerY = KSCREEN_HEIGHT * 0.8;
    secPL2.font = [UIFont systemFontOfSize:14];
    secPL2.textAlignment = NSTextAlignmentCenter;
    secPL2.textColor = KSRGBA(86, 86, 86, 1);
    secPL2.text = @"VIP专享购物 消费送好礼";
    [self.scrollView addSubview:secPL2];
    // 第三页的第一个label
    UILabel *thiPL = [[UILabel alloc] init];
    thiPL.frame = CGRectMake(0, 0, 200, 50);
    thiPL.centerX = secPL2.centerX + KSCREEN_WIDTH;
    thiPL.centerY = KSCREEN_HEIGHT * 0.75;
    thiPL.font = [UIFont systemFontOfSize:14];
    thiPL.textAlignment = NSTextAlignmentCenter;
    thiPL.textColor = KSRGBA(86, 86, 86, 1);
    thiPL.text = @"邀请好友注册 消费即可返利";
    [self.scrollView addSubview:thiPL];
}
...
-(UIButton *)guideButton{
    return MS_LAZY(_guideButton, ({
        ...
        [guideButton setBackgroundImage:[UIImage imageNamed:@"guideBG4"] forState:UIControlStateNormal];
