...
{
    // 2014-05-01
    NSDate *nowDate = [[NSDate date] zbn_dateWithYMD];
    
    // 2014-04-30
    NSDate *selfDate = [self zbn_dateWithYMD];
...
- (BOOL) zbn_isThisWeek
{
    return [self zbn_isSameWeekWithAnotherDate:[NSDate date]];
}
...
{
    return [[NSDateFormatter zbn_defaultDateFormatter] dateFromString:timestamp];
}
...
{
    return [[NSDateFormatter zbn_defaultDateFormatter] stringFromDate:[NSDate date]];
}
...
- (NSString *)zbn_formattedDateDescription
{
    ...
    if ([self zbn_isThisYear])
    {
        // 今年
        if ([self zbn_isToday]) {
            // 22:22
            dateFormatter = [NSDateFormatter zbn_dateFormatterWithFormat:@"HH:mm"];
        }else if ([self zbn_isYesterday]){
            // 昨天 22:22
            dateFormatter = [NSDateFormatter zbn_dateFormatterWithFormat:@"昨天 HH:mm"];
        }else if ([self zbn_isThisWeek]){
            // 星期二 22:22
            dateFormatter = [NSDateFormatter zbn_dateFormatterWithFormat:[NSString stringWithFormat:@"%@ HH:mm" , [self zbn_weekDay]]];
        }else{
            // 2016年08月18日 22:22
            dateFormatter = [NSDateFormatter zbn_dateFormatterWithFormat:@"yyyy年MM月dd日 HH:mm"];
        }
        
    }else{
        // 非今年
        dateFormatter = [NSDateFormatter zbn_dateFormatterWithFormat:@"yyyy年MM月dd日"];
    }
...
- (NSString *)zbn_string_yyyy_MM_dd {
    return [self zbn_string_yyyy_MM_dd:[NSDate date]];
}
