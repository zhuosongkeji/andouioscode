...
//            [titleAddress appendString:[[NSString alloc]initWithFormat:@"%@",@""]];
            if (i == count - 1) {
                [titleID appendString:[[NSString alloc]initWithFormat:@" %@",self.titleIDMarr[i]]];
//                [titleID appendString:[[NSString alloc]initWithFormat:@"%@",@""]];
            }
            else{
                [titleID appendString:[[NSString alloc]initWithFormat:@" %@",self.titleIDMarr[i]]];
            }
...
-(void)setupOneTableView:(NSInteger)btnTag{
    ...
    contentView.contentInset = UIEdgeInsetsMake(0, 0, 80, 0);
...
//        NSString * UserID = [[NSString alloc]initWithFormat:@"%ld",self.userID];
        if (addressID == 1) {
            //获取省份的URL
            addressUrl = @"http://andou.zhuosongkj.com/index.php/api/common/district";
            //请求省份需要传递的参数
            parameters = @{};
        } else if(addressID == 2){
            //获取市区的URL
...
//            parameters = @{@"province_id" : provinceIdOrCityId};
            parameters = @{@"id" : provinceIdOrCityId};
        }
        else if(addressID == 3){
            //获取县的URL
            addressUrl = @"http://andou.zhuosongkj.com/index.php/api/common/district";
...
//            parameters = @{@"city_id" : provinceIdOrCityId};
    ...
    [FKHRequestManager sendJSONRequestWithMethod:RequestMethod_GET pathUrl:addressUrl params:parameters complement:^(ServerResponseInfo * _Nullable serverInfo) {
        NSLog(@"===++++++++++==%@",serverInfo.response);
        NSArray *arr = [[NSArray alloc]init];
            switch (addressID) {
            case 1:
                //拿到省列表
                    arr =  serverInfo.response[@"data"];
...
//    //网络请求
//    [HttpRequest requestWithURLString:addressUrl parameters:nil type:HttpRequestTypeGet success:^(id responseObject) {
//        NSLog(@"---------+++++------");
...
-(void)caseCityArr:(NSArray *)cityArray{
    if (cityArray.count > 0){
        [self.cityMarr removeAllObjects];
        for (int i = 0; i < cityArray.count; i++) {
            NSDictionary *dic1 = cityArray[i];
            CityModel *cityModel = [CityModel yy_modelWithDictionary:dic1];
            [self.cityMarr addObject:cityModel];
        }
        if (self.tableViewMarr.count >= 2){
