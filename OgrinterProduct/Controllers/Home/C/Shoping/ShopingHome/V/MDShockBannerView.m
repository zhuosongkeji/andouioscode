    ...
    if (scrollView.isDragging || scrollView.isDecelerating){
        ...
        if (_lastContentOffset > curretContentOffset){
            ...
            [UIView animateWithDuration:0.5 animations:^{
                ...
                [self.rightView setRadius:(KSCREEN_WIDTH * _draggingIndex - curretContentOffset)*2   direction:MDBannerSrollDirectionLeft];
            }];
        }
    }else{
        ...
        if (_lastContentOffset > curretContentOffset){
            ...
            [UIView animateWithDuration:0.5 animations:^{
                [self.leftView setRadius:0 direction:MDBannerSrollDirectionRight];
                [self.rightView setRadius:(curretContentOffset - KSCREEN_WIDTH * _autoScrollIndex)*2   direction:MDBannerSrollDirectionLeft];
            }];
        }
...
#pragma Get
-(CGFloat)naxH{
    return kStatusBarAndNavigationBarH;
}
...
-(UICollectionViewFlowLayout *)flowLayout{
    if (_flowLayout == nil){
        ...
        _flowLayout.itemSize = CGSizeMake(self.width, (self.height - self.naxH));
...
-(UICollectionView *)banner{
    if (_banner == nil){
        _banner = [[UICollectionView alloc] initWithFrame:CGRectMake(0, self.naxH + 10, self.width, self.height - self.naxH) collectionViewLayout:self.flowLayout];
...
-(MDMaskView *)leftView{
    if (_leftView == nil){
        _leftView = [[MDMaskView alloc] initWithFrame:CGRectMake(-10, 20, self.width + 10, 4*(self.height - self.naxH)/5)];
...
-(MDMaskView *)rightView{
    if (_rightView == nil){
        _rightView = [[MDMaskView alloc] initWithFrame:CGRectMake(0, 20, self.width + 10, 4*(self.height - self.naxH)/5)];
    }
    return _rightView;
}
...
-(UIView *)bannerMaskView{
    if (_bannerMaskView == nil) {
        _bannerMaskView = [[UIView alloc] initWithFrame:CGRectMake(0, self.naxH+10, self.width, self.height - self.naxH)];
//        [_bannerMaskView setBackgroundColor:[UIColor redColor]];
    }
