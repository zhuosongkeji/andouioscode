...
@implementation ZBNRefreshHeader
...
- (UIView *)bgView {
    if (!_bgView) {
        [self addSubview:_bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, KSCREEN_WIDTH * 2, 60)]];
        for (int i = 0; i < 2; i ++) {
            UIImageView *image = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"refreshbg"]];
            image.frame = CGRectMake(KSCREEN_WIDTH * i, 0, KSCREEN_WIDTH, 60);
...
- (void)prepare {
    ...
    arr = idleImages;
    [self setImages:arr duration:idleImages.count * 0.1 forState:MJRefreshStateIdle];
    [self setImages:arr duration:idleImages.count * 0.1 forState:MJRefreshStatePulling];
    [self setImages:arr duration:idleImages.count * 0.1 forState:MJRefreshStateRefreshing];
    [self setImages:arr duration:idleImages.count * 0.1 forState:MJRefreshStateWillRefresh];
...
- (void)setPullingPercent:(CGFloat)pullingPercent {
    ...
    self.customImageV.frame = CGRectMake(KSCREEN_WIDTH * 0.2  * pullingPercent ,  60 * (1 - pullingPercent), 60 * pullingPercent, 60 * pullingPercent);
...
- (void)endRefreshing {
    ...
    [UIView animateWithDuration:1.3f animations:^{
        weakSelf.customImageV.frame = CGRectMake(KSCREEN_WIDTH + 10, 60 * 0.00001, 60 * 0.9999, 60 * 0.9999);
...
- (CABasicAnimation *)creatAnimation {
    ...
    animation.toValue = [NSValue valueWithCGPoint:CGPointMake(-KSCREEN_WIDTH, 0)];
...
- (UILabel *)customStatusLable {
    if (!_customStatusLable) {
        [self addSubview:_customStatusLable  = [[UILabel alloc] initWithFrame:CGRectMake(0, 40, KSCREEN_WIDTH, 20)]];
