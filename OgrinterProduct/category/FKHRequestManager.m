...
@implementation FKHRequestManager
...
+(void)sendJSONRequestWithMethod:(RequestMethod )requestMethod
    ...
    dataTask =  [client dataTaskWithRequest:[request copy] uploadProgress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (error) {
...
#endif
            [FKHRequestManager handleFailTask:dataTask error:error complement:result];
        }else{
            // 服务器响应 log 数据
            ...
            NSLog(@"URL Value=%@",serverUrl);
#endif
            [FKHRequestManager handleSuccessTask:dataTask responseObject:responseObject complement:result];
        }
    }];
...
+ (void)uploadFileWithPath:(NSString *__nonnull)path
    ...
    [FKHRequestManager startRequestByMethod:RequestMethod_POST
...
+ (void)requestWidth:(RequestMethod)method
    ...
    [FKHRequestManager startRequestByMethod:method
                                   ...
                                   requestType:RequestType_REQUEST
                                    ...
                                    complement:^(ServerResponseInfo *serverInfo) {
                                        ...
                                        if (serverInfo.httpCode == 302 && count < 2) {
                                            count ++;
                                            [FKHRequestManager requestWidth:method requestManager:manager pathUrl:pathUrl params:params requestCount:count complement:result];
...
+ (void)startRequestByMethod:(RequestMethod)requestMethod
...
#endif
        FKHRequestOperation<FKHOperationAdapter> *requestOperation = [[FKHRequestOperation alloc]initOperationWithTask:^{
            [FKHRequestManager handleSuccessTask:task responseObject:responseObject complement:result];
        }];
...
#endif
        FKHRequestOperation<FKHOperationAdapter> *requestOperation = [[FKHRequestOperation alloc]initOperationWithTask:^{
            [FKHRequestManager handleFailTask:task error:error complement:result];
        }];
        [manager.requestQueue addOperation:requestOperation];
    ...
    switch (requestType) {
        ...
        {
            [FKHRequestManager startRequestByMethod:requestMethod requestManager:client requestPath:serverUrl bodyParams:params successBlock:successBlock failureBlock:failureBlock];
        }
            break;
        case RequestType_UPLOAD:
        {
            [FKHRequestManager uploadFileWithManager:client requestPath:serverUrl bodyParams:params fileArray:fileArray fileType:fileType successBlock:successBlock failureBlock:failureBlock];
        }
            break;
...
+ (void)handleSuccessTask:(NSURLSessionDataTask * _Nonnull )task responseObject:(id  _Nullable)responseObject complement:(resultBlock)result{
    ...
    infoData.responseHeader = [FKHRequestManager josnSerialization:response.allHeaderFields];
...
+ (void)uploadFileWithManager:(AFHttpClientManager *)manager
    ...
    [manager POST:pathUrl parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (fileType == FileType_Image) {
            [FKHRequestManager imgFormdata:formData fileArray:fileArray];
        }else{
            [FKHRequestManager vedioFormdata:formData fileArray:fileArray];
        }
...
+ (void)getObjectFromJSONData:(NSData *)data
    ...
    if (data.length != 0)
    {
        ...
        if (!jsonObject)
        {
            ...
            jsonObject = [FKHRequestManager replaceUnicode:strJSON];
    ...
    if ([strResponse containsString:@"\\u"]) {
        strResponse = [FKHRequestManager replaceUnicode:strResponse];
    }
